function __print_squid_functions_help() {
cat <<EOF
Additional Squid-OS functions:
...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/squid/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        if (adb shell getprop ro.squid.device | grep -q "$SQUID_BUILD"); then
...
function githubremote()
{
    ...
    local REMOTE=$(git config --get remote.squid.projectname)

    local PROJECT=$(echo $REMOTE | sed -e "s#platform/#android/#g; s#/#_#g")

    git remote add github https://github.com/Squid-OS/$PROJECT
...
function installboot()
{
    ...
    if (adb shell getprop ro.squid.device | grep -q "$SQUID_BUILD");
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/squid/build/tools/repopick.py $@
}
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/squid/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
