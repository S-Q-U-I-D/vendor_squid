...
# Copyright (C) 2019 The Squid-OS Project
#
...
function setup_vendor() {
    ...
    export SQUID_ROOT="$3"
    if [ ! -d "$SQUID_ROOT" ]; then
        echo "\$SQUID_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$SQUID_ROOT/$OUTDIR" ]; then
        mkdir -p "$SQUID_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$SQUID_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$SQUIDROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$SQUID_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            ...
            printf "# Copyright (C) 2017 The Squid-OS Project\n" >> $1
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$SQUID_ROOT"/vendor/squid/build/tools/smali/baksmali.jar
        export SMALIJAR="$SQUID_ROOT"/vendor/squid/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$SQUID_ROOT"/vendor/squid/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$SQUID_ROOT"/vendor/squid/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if [ ! -f "$SQUID_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$SQUID_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$SQUID_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$SQUID_ROOT"/vendor/squid/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$SQUID_ROOT"/"$OUTDIR"/radio
