...
func init() {
	android.RegisterModuleType("squid_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = squidExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = squidExpandVariables(ctx, dep_file)
	...
	cmd := squidExpandVariables(ctx, String(g.properties.Cmd))
